 ...
 * @target MZ
 *
 * @plugindesc TRP_SkitMZの設定用プラグイン。TRP_SkitMZより上に配置。
 * @author Thirop
 *
 * @help
 ...
 * 
 * 【更新履歴】
 * 1.01 2020/12/7  「ネームボックスにセット」を追加
 * 1.00 2020/11/10 初版
 * 
 * @param categoryBasic
 * @text 【必須設定】
 ...
 * @default ["{\"name\":\"アイナ\",\"inputName\":\"アイナ\",\"fileName\":\"aina\",\"pose\":\"[\\\"{\\\\\\\"name\\\\\\\":\\\\\\\"normal\\\\\\\",\\\\\\\"displayOffsetY\\\\\\\":\\\\\\\"250\\\\\\\",\\\\\\\"animationOffsetX\\\\\\\":\\\\\\\"0\\\\\\\",\\\\\\\"animationOffsetY\\\\\\\":\\\\\\\"0\\\\\\\",\\\\\\\"defaultExpressionMultiLayer\\\\\\\":\\\\\\\"0\\\\\\\"}\\\"]\"}"]
 ...
 * @param useNameBox
 * @text 　ネームボックス使用
 * @desc ON/trueでメッセージのネームボックスを使用して発言キャラを識別します。(デフォルト値：ON)
 * @type boolean
 * @default true
 * @parent categoryBasic
 *
 * @param autoDetectMobName
 * @text 　モブキャラ自動識別
 * @desc ネームボックス使用ONでのみ有効。ON/trueで登録キャラ以外の発言時に表示キャラをアンフォーカス(デフォルト値：ON)
 * @type boolean
 * @default true
 * @parent categoryBasic
 * 
 ...
 * @text 　デフォルト表情の使用
 * @desc デフォルトで「ポーズ名_default.png」の表情を表示。多層レイヤ使用時は無効
 * @type boolean
 * @default true
 * @parent categoryBasic
 * 
 ...
 * @parent categoryDisplayPosition
 * 
 * @param categoryAutoMode
 * @text 【自動モード設定】
 * @default ────────────────
 *
 * @param defaultAutoMode
 * @text 自動モードON/OFFの初期値
 * @desc ON/trueにすると初期状態で自動モードがON。(ON/OFFはコマンドで切り替え可能)
 * @type boolean
 * @default true
 * @parent categoryAutoMode
 *
 * @param defaultShowCommand
 * @text 登場コマンド
 * @desc 登場時に使用するコマンド。マクロを使用する際はテキストタブで設定。（退出は「デフォルトの退出コマンド」で指定）
 * @type select
 * @option フェードイン
 * @value 0
 * @option ムーブイン
 ...
 * @parent categoryAutoMode
 *
 * @param autoModePositions
 * @text 表示位置の優先順
 * @desc [左,右]だと左、右の順に２キャラ、[左]だと常に左に１キャラのみ表示。
 * @type string[]
 * @default ["左","右"]
 * @parent categoryAutoMode
 *
 * @param autoShowWait
 * @text 自動表示時のウェイト
 * @desc 自動表示時のウェイト。0で無し、-1で表示完了まで、1以上で任意のフレームウェイト。
 * @type number
 * @min -1
 * @default 4
 * @parent categoryAutoMode
 *
 *
 * @param categoryMultiLayer
 * @text 【多層レイヤー使用の設定】
 * @default ────────────────
 *
 * @param useMultiLayer
 * @text 表情の多層レイヤー使用
 * @desc 目や鼻、口などパーツごとにレイヤー分けした画像を使用
 * @type boolean
 * @default false
 * @parent categoryMultiLayer
 *
 * @param useSpritesheets
 * @text スプライトシート使用
 * @desc 多層レイヤー使用時にスプライトシートを使用。
 * @type boolean
 * @default false
 * @parent categoryMultiLayer
 *
 * @param spritesheets
 * @text スプライトシートファイル名
 * @desc ファイル名をカンマ(,)区切りで指定。省略で全キャラ・ポーズ読み込み。キャラ名_ポーズ名.jsonの形式
 * @type text
 * @parent categoryMultiLayer
 *
 * 
 ...
 * @default ================================
 *
 * @param disableLookaheadMessage
 * @text 表情先読み無効
 * @desc ON/trueにすると直後のメッセージ先頭でのポーズ・表情変更の先読みを無効
 * @type boolean
 * @default false
 *
 * @param setNameBox
 * @text ネームボックス自動セット
 * @desc ON(true)にするとネームボックス未使用での入力に対し、ネームボックスに名前を表示。
 * @default false
 * @type boolean
 *
 * @param macro
 ...
 * @desc 《スライドイン》コマンドの設定
 * @type struct<SlideIn>
 * @default {"speed":"3","slideLength":"0.5","wait":"true","easeType":"3","pushIn":"true"}
 * @parent categoryAppearCommands
 *
 * @param moveIn
 * @text 　《ムーブイン》の設定
 ...
 * @text デフォルトの退出コマンド
 * @desc デフォルトの退出コマンド。マクロを使用する際はテキストタブで設定。
 * @type select
 * @option フェードアウト
 * @value 0
 * @option ムーブアウト
 ...
 * @default {"speed":"4","slideLength":"0.5","wait":"true","easeType":"3"}
 * @parent categoryDisappearCommands
 *
 * @param moveOut
 * @text 　《ムーブアウト》の設定
 ...
 * @text アニメーション表示X
 * @desc アニメーション表示の水平方向にずらす量（デフォルト値：0）
 * @type number
 * @min -10000
 * @default 0
 *
 ...
 * @text アニメーション表示Y
 * @desc アニメーション表示の垂直方向にずらす量（デフォルト値：0）
 * @type number
 * @min -10000
 * @default 0
 *
 * @param defaultExpressionMultiLayer
 * @text 　デフォルト表情ID(多層レイヤー用)
 * @desc 多層レイヤー使用時のデフォルトの表情ID
 * @type number
 * @default 0
 */
 ...
 * @param relative
 * @text 絶対/相対[デフォ値]
 * @desc 拡大率指定の相対/絶対のデフォルト値。初期値は絶対(0)
 * @type select
 * @option 絶対(0)
 * @value 0
 * @option 相対(1)
 ...
 * @param opacity
 * @text 不透明度[デフォ値]
 * @desc 不透明度変更のデフォルト値。初期値は0。(255で不透明)
 * @type number
 * @default 0
 *
 * @param relative
 * @text 絶対/相対[デフォ値]
 * @desc 不透明度指定の相対/絶対のデフォルト値。初期値は絶対値(0)
 * @type select
 * @option 絶対(0)
 * @value 0
 * @option 相対(1)
 ...
 * @param wait
 * @text ウェイト[デフォ値]
 * @desc 不透明度コマンドのウェイトのデフォルト値(初期値false)
 * @type boolean
 * @default false
 *
 * @param easeType
...
TRP_CORE.skitParameters = PluginManager.parameters('TRP_SkitMZ_Config');